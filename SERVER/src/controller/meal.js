...
class Meal {
  static addMeal(req, res) {
    ...
    if (req.files === null || req.files.image === null) {
      ...
      Meals.create({
        userId, name, price, quantity, category, image,
      })
        .then(meal => res.status(201)
          .json({ success: true, msg: 'meal added successfully', data: meal })
          .end()).catch((err) => {
          res.status(409)
  ...
  static async addMeals(req, res) {
    ...
    if (bulkMeals.length === 0 || images.length === 0) {
      ...
      Meals.bulkCreate(mealBulk).then(() => {
        res.status(201)
          .json({ success: true, msg: 'meals added successfully' }).end();
      });
    }
  }
  static modifyMeal(req, res) {
    ...
    Meals.findById(id).then(meal => meal.update({
      ...
      res.status(200).send({
        success: true, data: updatedMeal, msg: 'meal modified successfully',
      });
    })).catch(err =>
      res.status(404)
